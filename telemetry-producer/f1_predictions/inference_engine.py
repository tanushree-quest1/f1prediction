"""
Core polling and inference loop for the F1 telemetry pipeline.

What it does:
Connects to ClickHouse, polls the `raw_telemetry` table for the latest driver events, maintains an in-memory 
state of the race (laps, gaps, tires), and invokes machine learning models (XGBoost/scikit-learn) to generate predictions.

Why it exists:
Serves as the central "brain" of the application, bridging the gap between raw data ingestion and real-time AI insights. 
It formats the predictions and posts them to the UI backend for rendering.
"""
