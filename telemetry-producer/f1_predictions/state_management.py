...
What it does:
Provides utility functions to update and maintain the in-memory race state (e.g., detecting when a 
new lap starts, updating gaps between drivers, or tracking active Safety Cars).

Why it exists:
Prevents the main `inference_engine.py` from becoming cluttered with boilerplate state transition logic, 
keeping the core loop focused on high-level orchestration.
"""
import numpy as np
