"""
Database interaction module for ClickHouse.

What it does:
Handles all read and write operations to the ClickHouse database, including fetching historical race data, 
loading pre-race contexts, and executing queries for the inference engine.

Why it exists:
Abstracts the ClickHouse SQL syntax and connection management away from the core inference loop, ensuring 
clean separation of concerns (Data Access Layer).
"""
import glob
import os
import re
...
def load_prerace_from_clickhouse(ch, event, year, session, race_laps, canonical_event_name, normalize_driver_code):
    ...
    if not num_to_code:
        ...
        # Only allow alphanumeric, underscores, and hyphens in event names
        csv_event = re.sub(r'[^a-zA-Z0-9_-]', '', csv_event)
        if not csv_event:
            pass  # Skip if event becomes empty after sanitization
        # Validate year is a reasonable integer
        try:
            year_int = int(year)
            if year_int < 2000 or year_int > 2100:
                return pre_race_ctx, num_to_code, 0  # Skip if year is out of reasonable range
        except (ValueError, TypeError):
            return pre_race_ctx, num_to_code, 0  # Skip if year is invalid
        ...
        for d in search_dirs:
            if not d or not os.path.isdir(d):
                continue
            # Security: Use os.path.realpath to resolve any path traversal attempts
            d_real = os.path.realpath(d)
            csv_pattern = os.path.join(d_real, f"{year}_{csv_event}_qualifying_results.csv")
            # Security: Verify the resolved path is still within the search directory
            csv_real = os.path.realpath(csv_pattern)
            if os.path.commonpath([d_real, csv_real]) != d_real:
                print(f"  âš  Blocked potential path traversal: {csv_pattern}")
                continue
            matches = glob.glob(csv_real)
            if not matches:
                continue
            try:
...
def load_cumulative_times(ch, event, year, session, up_to_lap, num_to_code, pit_stop_loss_s):
    if up_to_lap < 1:
        return {}
    # Security: Validate pit_stop_loss_s to prevent SQL injection
    try:
        pit_stop_loss = float(pit_stop_loss_s)
        if pit_stop_loss < 0 or pit_stop_loss > 1000:  # Reasonable range for pit stop loss
            pit_stop_loss = 23.0  # Default to safe value if out of range
    except (ValueError, TypeError):
        pit_stop_loss = 23.0  # Default to safe value if invalid
    
    df = ch.query_df(
        f"""
        select driver_num,
               sum(case when is_pit = 1 then lap_time_s + {pit_stop_loss} else lap_time_s end) as total_time_s,
