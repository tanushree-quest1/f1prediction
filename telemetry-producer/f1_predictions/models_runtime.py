"""
Model loading and execution runtime.

What it does:
Loads pre-trained machine learning models (XGBoost, scikit-learn) from disk and manages 
their execution during the inference loop.

Why it exists:
Isolates the complexity of model deserialization, versioning, and inference calls from the 
main polling loop.
"""
import hashlib
import json
...
def _load_pickle(name: str, model_dir: str, manifest_path: str, strict: bool, status, fail, warn):
    ...
    # Security: Check file size to prevent DoS attacks (limit to 500MB)
    file_size = os.path.getsize(path)
    MAX_PICKLE_SIZE = 500 * 1024 * 1024  # 500MB
    if file_size > MAX_PICKLE_SIZE:
        fail(f"âœ— Model file too large: {path} ({file_size} bytes > {MAX_PICKLE_SIZE} bytes)")
        raise SystemExit(1)
    
    _verify_artifact_hash(path, manifest_path, strict, warn)
    
    # Security: Use restricted pickle loading to prevent code execution
    class SafeUnpickler(pickle.Unpickler):
        def find_class(self, module, name):
            # Allow only safe classes from standard library and known ML libraries
            safe_modules = {
                'builtins', 'collections', 'collections.abc', 'copyreg', 'io',
                'numpy', 'pandas', 'sklearn', 'xgboost', 'joblib'
            }
            # Allow basic types and numpy/sklearn/xgboost classes
            if module in safe_modules or module.startswith('numpy.') or module.startswith('sklearn.') or module.startswith('xgboost.'):
                return super().find_class(module, name)
            # Block everything else
            raise pickle.UnpicklingError(f"Blocked attempt to unpickle unsafe class: {module}.{name}")
    
    try:
        with open(path, "rb") as f:
            return SafeUnpickler(f).load()
    except pickle.UnpicklingError as e:
        fail(f"âœ— Security error loading pickle: {e}")
        raise SystemExit(1)
...
def load_runtime_artifacts(model_dir: str, manifest_path: str, strict: bool, status, fail, warn) -> RuntimeArtifacts:
    ...
    if os.path.isfile(pit_thr_path):
        try:
            ...
            file_size = os.path.getsize(pit_thr_path)
            if file_size > 1024:  # Pit threshold should be very small (< 1KB)
                warn(f"âš  Pit threshold file suspiciously large: {file_size} bytes")
            _verify_artifact_hash(pit_thr_path, manifest_path, strict, warn)
            with open(pit_thr_path, "rb") as f:
                # Use safe unpickler for consistency
                class SafeUnpickler(pickle.Unpickler):
                    def find_class(self, module, name):
                        safe_modules = {'builtins', 'collections', 'collections.abc', 'copyreg', 'io'}
                        if module in safe_modules:
                            return super().find_class(module, name)
                        raise pickle.UnpicklingError(f"Blocked attempt to unpickle unsafe class: {module}.{name}")
                pit_alert_threshold = float(SafeUnpickler(f).load())
