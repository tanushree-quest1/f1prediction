<#
.SYNOPSIS
End-to-End Orchestrator for the F1 Telemetry Pipeline.

.DESCRIPTION
What it does:
Bootstraps the entire environment in a single command. It provisions the Docker network, starts the 
database and ML infrastructure (ClickHouse, Kafka, LibreChat), compiles the Java Producer, and 
launches the Python Inference Engine and UI.

Why it exists:
Provides a seamless, one-click developer experience, eliminating the need to manually execute 
a dozen commands across multiple terminal windows.
#>
param (
    [string]$EventName = "Miami_Grand_Prix",
    [int]$RaceYear = 2026,
    [string]$SessionType = "R",
    [int]$Speed = 10
)

$ErrorActionPreference = "Stop"

Write-Host "F1 $RaceYear $($EventName.Replace('_', ' ').ToUpper()) -- END-TO-END PIPELINE" -ForegroundColor Cyan
...
$EVENT_DISPLAY = $EventName.Replace("_", " ")
$YEAR          = $RaceYear
$SESSION       = $SessionType
$SPEED_FACTOR  = $Speed
$UI_PORT       = 8000
...
Write-Host "[BUILD] Java telemetry producer will be built by Docker..." -ForegroundColor Yellow
Write-Host ""
