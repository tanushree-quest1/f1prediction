"""
FastAPI backend for the real-time WebSocket dashboard.

What it does:
Provides a REST API to receive JSON payloads from the Inference Engine, maintains an aggregated 
in-memory state of the race, and broadcasts that state to all connected web clients via WebSockets.

Why it exists:
Decouples the heavy machine learning inference logic from the high-concurrency client delivery. 
It ensures that hundreds of connected browsers can receive 60Hz telemetry updates without bogging 
down the ML engine.
"""
import asyncio
import json
...
def _require_internal_auth(x_internal_token: Optional[str]) -> None:
    ...
    if not hmac.compare_digest(x_internal_token.encode('utf-8'), INTERNAL_API_TOKEN.encode('utf-8')):
        raise HTTPException(status_code=401, detail="Unauthorized")
...
async def update_predictions(data: dict, x_internal_token: Optional[str] = Header(default=None)):
    ...
    # Security: Validate input data size to prevent DoS
    if data and len(str(data)) > 10_000_000:  # 10MB limit
...
async def update_commentary(items: List[str], x_internal_token: Optional[str] = Header(default=None)):
    ...
    if len(items) > 100:  # Limit number of commentary items
        raise HTTPException(status_code=400, detail="Too many commentary items")
    # Sanitize each commentary item
    sanitized_items = []
    for item in items:
    ...
    async with commentary_lock:
        latest_commentary = (sanitized_items + latest_commentary)[:7]
