/**
 * Core event loop for synchronizing and streaming telemetry data.
 * 
 * What it does:
 * Maintains a priority queue of telemetry streams (one per driver), popping the chronologically 
 * earliest event across all drivers, sleeping to simulate real-time (adjusted by speed factor), 
 * and publishing the event to Kafka.
 * 
 * Why it exists:
 * Real F1 telemetry data comes in parallel for 20 drivers. This class ensures the data is 
 * replayed in strict chronological order across all drivers to accurately simulate a live race 
 * for the downstream ML inference engine.
 */
package f1producer;
